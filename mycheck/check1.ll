...
;     코드상 BB의 순서를 실제 logic flow와는 다른 순서로 배치해놓음.
...
; CHECK:        true2:
; CHECK-NEXT:     call i32 @nonrecur(i32 [[A]], i32 [[A]], i32 [[A]], i32 [[A]])
; CHECK-NEXT:     br label [[EXIT]]
; CHECK:        exit:
; CHECK-NEXT:     call i32 @nonrecur(i32 [[A]], i32 [[B]], i32 [[X]], i32 [[Y]])
; CHECK-NEXT:     ret i32 0
; CHECK:        true1:
; CHECK-NEXT:     [[D:%.*]] = add i32 [[A]], [[Y]]
; CHECK-NEXT:     call i32 @nonrecur(i32 [[D]], i32 [[A]], i32 [[A]], i32 [[D]]
; CHECK-NEXT:     [[COND3:%.*]] = icmp eq i32 [[D]], [[A]]
; CHECK-NEXT:     br i1 [[COND3]], label [[TRUE2:%.*]], label [[EXIT]]
; CHECK:        true:
; CHECK-NEXT:     [[C:%.*]] = add i32 [[A]], [[X]]
; CHECK-NEXT:     call i32 @nonrecur(i32 [[C]], i32 [[A]], i32 [[C]], i32 [[A]])
...
true2:
  call i32 @nonrecur(i32 %d, i32 %b, i32 %c, i32 %d)
  br label %exit
exit:
  call i32 @nonrecur(i32 %a, i32 %b, i32 %x, i32 %y)
  ret i32 0
true1:
  %d = add i32 %c, %y
  call i32 @nonrecur(i32 %d, i32 %b, i32 %c, i32 %d)
...
true:
  %c = add i32 %a, %x
  call i32 @nonrecur(i32 %c, i32 %b, i32 %c, i32 %b)
  %cond2 = icmp eq i32 %b, %c
  br i1 %cond2, label %true1, label %exit
}
declare i32 @nonrecur(i32, i32, i32, i32)
